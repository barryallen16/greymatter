...
| # | Repo | Issue | Action |
|---|---|---|---|
| 1 | `moviemod-scraper` | **`.env` was tracked in git** with live Telegram bot token (`8926550373:AAGoz…`) + private group chat ID — now gitignored (only `.env.example` tracked as of Sep 2026), but the secret is still in history | Revoke via @BotFather NOW; purge history with git-filter-repo |
...
| 22 | moviemod-scraper | **5/10** | ~1,980 (`moviescraper.py` 1,096 + `src/` 800) | ✅ 71 tests, ruff clean | 16 stars & battle-tested; Sep 2026 refactor split the old 1,639-line monolith into `src/` (`db/introspect/interactive/linkcheck/notify/helpers`) + questionary interactive CLI (search → season → scope → resolution) with stored-link validation/reuse, `uv` + Docker + `pyproject.toml`; still MySQL-only, piracy ethics — frame as "browser automation against adversarial targets" if at all |
...
| **moviemod-scraper (5)** | 16 stars, working, survives piracy-domain rotations via redirect-tracker resolution; Docker compose w/ MySQL healthcheck. Sep 2026: old 1,639-line monolith split into `src/` modules + questionary interactive CLI, 71 tests + ruff clean, `.env` now gitignored (secret still in history — purge it). Remaining: module-level scraping flow still heavy, MySQL-only, **old live token leak in history**, piracy ethics. Frame as "browser automation against adversarial targets" if at all. |
...
1. Revoke Telegram token + both OpenRouter keys; purge `.env` from moviemod-scraper history (current tree already gitignores it — history still leaks); scrub middleman PII; delete the four junk repos.
