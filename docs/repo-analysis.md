...
*All 34 public repos cloned, read, and analyzed line-by-line. August 2026. Updated 2026-09-01 to include `tamil-tokenizer` (pushed 2026-08-28); `kaggle-automation` re-audited 2026-09-13 to head `cc8f2d7` (Docker + CI + uv + 79 tests); `moviemod-scraper` re-audited 2026-09-20 to head `be30189` (src layout + interactive CLI + 71 tests, ruff clean).*
...
| # | Repo | Issue | Action |
|---|---|---|---|
| 1 | `moviemod-scraper` | **`.env` was tracked in git** with live Telegram bot token (`8926550373:AAGoz…`) + private group chat ID | ✅ Resolved Sep 20, 2026: token revoked via @BotFather, leak branch deleted, `main` + `origin/main` verified clean (`git log -S` + full-rev `git grep` empty) |
...
| 22 | moviemod-scraper | **5.5/10** | ~1,980 (`moviescraper.py` 1,096 + `src/` 800) | ✅ 71 tests, ruff clean | 16 stars, shipped + used; Sep 2026 refactor into `src/` (`db/introspect/interactive/linkcheck/notify/helpers`) + questionary CLI with stored-link validation/reuse; intentional single-producer crawl → parallel Selenium consumer pool; secret history resolved Sep 20. Still MySQL-only + piracy ethics — supporting bullet, frame as "browser automation against adversarial targets" |
...
| **moviemod-scraper (5.5)** | 16 stars, working, survives piracy-domain rotations via redirect-tracker resolution; Docker compose w/ MySQL healthcheck. Sep 2026: old 1,639-line monolith split into `src/` modules + questionary interactive CLI, 71 tests + ruff clean, `.env` gitignored + leaked token revoked with history verified clean. Architecture is deliberate single-producer crawl → N Selenium consumers over a shared link queue (sequential crawl avoids coordination complexity by design). Remaining: 1,096-line orchestrator, MySQL-only, piracy ethics. Best 4th bullet for Selenium/queue breadth — frame as "browser automation against adversarial targets". |
...
3. **CI + Docker now exist in 3 repos, tests in exactly 3 strong repos.** `tamil-tokenizer` (23 tests) + `kaggle-automation` (79 tests across 12 files + pinned GH Actions CI + multi-stage `Dockerfile` + compose, all Sep 5) + `moviemod-scraper` (71 tests + `ruff` clean, Sep 2026 src-layout refactor). No CI for `moviemod-scraper` by decision (single-user scraper; failure modes are site-side, untestable in Actions) — add CI to `tamil-tokenizer` next; it becomes the Rust template.
...
1. ~~Revoke moviemod-scraper Telegram token + purge its `.env` history~~ ✅ done Sep 20, 2026 (revoked, branch deleted, history verified clean). Still open: revoke both OpenRouter keys; scrub middleman PII; delete the four junk repos.
