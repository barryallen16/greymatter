...
```mermaid
graph TD
    subgraph The Universal CSE Fresher Trinity
        P1["1. DSA & Problem Solving<br/>(150–250 LeetCode Problems)"]
        P2["2. Core CS Fundamentals<br/>(DBMS + OS + Networks + LLD)"]
        P3["3. Language & Systems Mastery<br/>(Deep Internals of 1 Language)"]
    end
    
    P1 --> Gate1[Clears Online Assessment & Live Coding Round]
    P2 --> Gate2[Clears Technical Interview 1 & 2]
    P3 --> Gate3[Defends Architecture & Projects]
```
...
sequenceDiagram
    autonumber
    actor Candidate
    participant OA as Round 1: Online Assessment (OA)
    participant Tech1 as Round 2: DSA & Live Coding
    participant Tech2 as Round 3: LLD, CS Core & Projects
    participant HR as Round 4: Techno-Managerial / HR

    Candidate->>OA: Solves 2–3 Coding Questions + MCQs (90–120 mins)
    Note over OA: Elimination: ~85% filtered out
    OA-->>Candidate: Shortlist Invitation
    
    Candidate->>Tech1: Live Coding with Senior SDE (45–60 mins)
    Note over Tech1: Think aloud, derive time/space complexity, handle edge cases
    Tech1-->>Candidate: Technical Pass
    
    Candidate->>Tech2: Architecture, DBMS/OS/CN, Project Defense (60 mins)
    Note over Tech2: Deep code review, schema design, failure handling, LLD
    Tech2-->>Candidate: Leadership Endorsement
    
    Candidate->>HR: Behavioral, Cultural Fit, Salary & Bond Check (30 mins)
    HR-->>Candidate: Offer Letter Issued (> ₹5 LPA, 0 Bond)
```
