...
- KV key `jobfilter-jobs-v1` — scored jobs: `{id, title, company, location, job_url, site, source_type, description, score, match_reasons[], min_exp_years, max_exp_years, job_type, date_posted, discovered_at}` (`date_posted` = true posted date where a source gives one, else `""`; the jobs page falls back to `discovered_at` and badges rows posted/seen in 24h as NEW). Written by `job-filter/publish.py`, rendered by `jobs/index.html`.
