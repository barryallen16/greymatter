...
- KV key `jobfilter-jobs-v1` — scored jobs: `{id, title, company, location, job_url, site, source_type, description, score, match_reasons[], min_exp_years, max_exp_years, job_type, date_posted, discovered_at, judge?, judge_reason?}` (`date_posted` = true posted date where a source gives one, else `""`; `judge: "drop"` = the `/job-judge` skill's verdict from `job-filter/data/verdicts.json`, hidden by the page). The jobs page falls back to `discovered_at`, badges 24h rows NEW, and colors experience fit for a 0-YOE fresher (`fresher fit` only when the band starts at 0, `needs experience` for 1–2 starts, `exceeds fresher` past the gate ceiling, `exp unstated`) — deliberately stricter than the `engine/scorer.py` gate, which keeps 1–3 bands in the DB. Written by `job-filter/publish.py`, rendered by `jobs/index.html`.
